Litebitcoin Core version 0.13.2 is now available from:

  <https://download.litebitcoin.org/litebitcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a litebitcoin
...
ECDSA signatures inside Litebitcoin transactions now use validation using
...
Previous versions of Litebitcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Litebitcoin Core was to raise the default minimum
relay fee.

Litebitcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Litebitcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running litebitcoind without having to do any manual configuration.
...
Litebitcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Litebitcoin Core will relay transactions with
...
entirely in Litebitcoin Core 0.13, and it is at this time undecided whether the
...
Litebitcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Litebitcoin Core automatically creates a hidden service to listen on, without
manual configuration. Litebitcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Litebitcoin Core is listening, and
...
Litebitcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Litebitcoin
Core to use floating fees. By default, Litebitcoin Core will use floating
fees.
...
`litebitcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /litebitcoinrpc>
        ProxyPass http://127.0.0.1:19038/
        ProxyPassReverse http://127.0.0.1:19038/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass litebitcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< litebitcoinrpc:<password>
    </Location>
...
Restarting litebitcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in litebitcoin.conf
...
litebitcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/litebitcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Litebitcoin Core code base has
...
- `litebitcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `litebitcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Litebitcoin scripting system that in combination with
...
Litebitcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Litebitcoin Core.
...
The two phases are distinct in the Litebitcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Litebitcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Litebitcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Litebitcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of litebitcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of litebitcoins being spent is exactly
...
- **Increased security for multisig:** Litebitcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Litebitcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Litebitcoin's current ledger (called the UTXO set),
  ...
  note that Litebitcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Litebitcoin users to individually opt-in to almost any change in the Litebitcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Litebitcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Litebitcoin
client to Litebitcoin Core 0.13.2 to signal for segwit support. When a
...
Since Litebitcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `litebitcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `litebitcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
