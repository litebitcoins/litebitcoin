...
LBTCVER=($($LITEBITCOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $LITEBITCOIND $LITEBITCOINCLI $LITEBITCOINTX $LITEBITCOINQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${LBTCVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${LBTCVER[1]}//g" ${MANDIR}/${cmdname}.1
done
